...
Be concise and technical. Report measurements with units. When guiding wiring, reference specific header pins (e.g., "connect to J1-4 which is GPIO4").`,
      ...
      tools: {
        ...
        scope_measure: tool({
          description:
            "Quick scalar measurement from the oscilloscope — no full waveform capture needed. Use for fast DC voltage reads, peak-to-peak, RMS, or frequency measurements.",
          inputSchema: z.object({
            channel: z
              .number()
              .min(0)
              ...
              .describe("Scope channel (0=CH1, 1=CH2)"),
            measurement: z
              .enum(["dc", "peak_to_peak", "rms", "frequency"])
          ...
          execute: async ({ channel, measurement, v_range }) => {
            return await runnerCall("/scope/measure", {
              channel,
              measurement,
              v_range,
            });
